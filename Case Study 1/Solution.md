...
Some customers ordered more than one item on their first day at the store. However, since there was no time element to the order_date column, there is no clear way to accurately say which was actually first.
...
WITH purchase_after_membership AS (
				...
				JOIN menu m 
				ON s.product_id = m.product_id
				WHERE s.order_date >= me.join_date)  -- any order after join date was after membership
